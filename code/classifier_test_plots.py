...
def test_plots(pkl):
    ...
    n_test_protons = (df['class'] == 0).sum()
    n_test_gammas = (df['class'] == 1).sum()
...
#        fp[i] = df[(df['class'] == 0) & (df['gammaness'] >= thr)].count()[0]
#        fn[i] = df[(df['class'] == 1) & (df['gammaness'] <= thr)].count()[0]
    ...
    for i, thr in enumerate(r):
        p_c[i] = df[(df['class'] == 0) & (df['gammaness'] >= thr)].count()[0]
        g_c[i] = df[(df['class'] == 1) & (df['gammaness'] >= thr)].count()[0]
    ...
    accscore = accuracy_score(df['class'], df['gammaness'].round(), normalize=True)
    ...
    ax.hist(df_protons['gammaness'], bins=100, density=True, label='Protons', alpha = 0.8, edgecolor='gray', linewidth=0.9)
    #ax.plot(fn / n_test_gammas, label="Gammas")
    ax.hist(df_gammas['gammaness'], bins=100, density=True, label='Gammas', alpha=0.8, edgecolor='gray', linewidth=0.9)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--pkl', type=str, default='', help='Pickle test file.', required=True)
